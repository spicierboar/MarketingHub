    ...
    <ol className="mb-6 flex flex-wrap items-center gap-2 text-sm">
      {steps.map((s, i) => (
        <li key={s.key} className="flex items-center gap-2">
          <span className={"flex h-7 w-7 items-center justify-center rounded-full text-xs font-semibold " + (i <= idx ? "bg-primary/20 text-primary ring-2 ring-primary" : "bg-muted text-muted-foreground")}>
            {i + 1}
          </span>
...
export default async function NewClientPage({
  ...
  searchParams: Promise<{ step?: string; companyId?: string; addons?: string; paid?: string; checkout?: string; clientEmail?: string }>;
}) {
  ...
  const selectedAddons = (params.addons ?? "").split(",").map((s) => s.trim()).filter(isAddonId) as AddonId[];
  const checkoutQueue = params.checkout === "success" && params.paid && isAddonId(params.paid)
  ...
  const unpaidAddon = step === "checkout" && companyId ? await nextUnpaidAddon(companyId, checkoutQueue) : null;

  return (
    <div>
      <PageHeader title="New client" description="Field sales wizard ΓÇö company, add-ons, client portal login." />
      <div className="mx-auto max-w-3xl p-6">
        <Card><CardContent className="p-6">
          <Stepper step={step} />
          {step === "business" && (
            <form action={saveBusinessStepAction} className="space-y-4">
          ...
          {step === "addons" && company && (
            <form action={saveAddonsStepAction} className="space-y-4">
              ...
              {ADDON_ORDER.map((id) => (
                <label key={id} className="flex gap-2 border rounded p-3">
                  <input type="checkbox" name="addonId" value={id} />
                  <span>{ADDONS[id].icon} {ADDONS[id].name} ΓÇö ${ADDONS[id].priceAudMonthly}/mo</span>
                </label>
              ))}
              <div className="flex gap-2"><Button type="submit" variant="secondary" formAction={skipAddonsAction}>Skip</Button><Button type="submit">Continue</Button></div>
            </form>
          )}
          {step === "checkout" && company && unpaidAddon && (
            <form action={startAddonCheckoutAction} className="space-y-4">
              ...
              <p>{ADDONS[unpaidAddon].name} for {company.name}</p>
              <Button type="submit">{stripeConfigured() ? "Stripe checkout" : "Enable (demo)"}</Button>
              <Button type="submit" variant="secondary" formAction={skipCheckoutAction}>Skip</Button>
            </form>
          )}
          {step === "provision" && company && (
            <form action={provisionClientAction} className="space-y-4">
              <input type="hidden" name="companyId" value={company.id} />
              <Field label="Client name" htmlFor="name"><Input id="name" name="name" required /></Field>
              <Field label="Client email" htmlFor="email"><Input id="email" name="email" type="email" required /></Field>
          ...
          {step === "done" && company && (
            <div className="text-center space-y-4">
              ...
              {params.clientEmail && <p className="text-sm text-muted-foreground">{params.clientEmail} ΓÇö magic link at /login</p>}
              <Link href={`/companies/${company.id}`}><Button variant="secondary">View company</Button></Link>
              <Link href="/sales/new-client"><Button>Add another</Button></Link>
            </div>
          )}
        </CardContent></Card>
      </div>
