...
async function handle(req: NextRequest) {
  ...
  const [iso, portal, reports, publicApi, crm, cms, funnel, loyalty, aiMos] = await Promise.all([
  ...
  const checks = [...iso.checks, ...portal.checks, ...reports.checks, ...publicApi.checks, ...crm.checks, ...cms.checks, ...funnel.checks, ...loyalty.checks, ...aiMos.checks];
  const failed = checks.filter((c) => !c.ok).length;
  const report = {
    ok: iso.ok && portal.ok && reports.ok && publicApi.ok && crm.ok && cms.ok && funnel.ok && loyalty.ok && aiMos.ok,
    passed: checks.length - failed,
    failed,
    purgeFailed: [...iso.purgeFailed, ...portal.purgeFailed, ...reports.purgeFailed, ...publicApi.purgeFailed, ...crm.purgeFailed, ...cms.purgeFailed, ...funnel.purgeFailed, ...loyalty.purgeFailed, ...aiMos.purgeFailed],
    durationMs: iso.durationMs + portal.durationMs + reports.durationMs + publicApi.durationMs + crm.durationMs + cms.durationMs + funnel.durationMs + loyalty.durationMs + aiMos.durationMs,
    checks,
  };
