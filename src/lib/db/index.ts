...
export async function listRagKnowledgeSources(
  ...
  if (isSupabaseConfigured()) {
    return supabaseRepo.listRagKnowledgeSources(companyId, includeInactive);
  }
  return (db().ragKnowledgeSources ?? [])
    .filter((s) => {
      if (s.companyId !== companyId) return false;
      if (!includeInactive) return s.status === "approved";
      return true;
    })
    .sort((a, b) => b.updatedAt.localeCompare(a.updatedAt));
}

export async function getRagKnowledgeSource(
  sourceId: string,
...
export async function getRagKnowledgeVersion(
  versionId: string,
